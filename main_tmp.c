...
void	ms_line_str_tokenizer(t_cmd *cmd, char *line)
{
	...
	while (line[i])
	{
		...
		else if (line[i] == 34 || line[i] == 39)
		{
			...
			i += k;
			if (line[i] == 34 && i < quote)
			{
				while (line[i] != 34)
					i++;
			}
			else if (line[i] == 39 && i < quote)
			{
				while (line[i] != 39)
					i++;
			}
			...
			{
				while (!(line[i + m] == 39 || line[i + m] == 34) \
				&& line[i + m] != ' ' && line[i + m] && line[i + m] != '|' \
				&& line[i + m] != '>' && line[i + m] != '<' && line[i + m])
					m++;
				m++;
			...
			if (line[i] == 34 || line[i] == 39)
			{
				...
				if (tmp == NULL)
					exit (1);
				ft_strlcpy(tmp, line + i, k);
				ft_strlcat(cmd->line_split[line_i], tmp, i + k);
				free(tmp);
			}
			i += k;
			if (line[i] == 34 && i < quote)
			...
			if (line[i] != ' ' && line[i] != '|' && line[i] != '>' && line[i] != '<' && line[i])
			{
				...
				if (tmp == NULL)
					exit (1);
				ft_strlcpy(tmp, line + i, m);
				ft_strlcat(cmd->line_split[line_i], tmp, ft_strlen(cmd->line_split[line_i]) + m);
				free(tmp);
			}
			line_i++;
		}
